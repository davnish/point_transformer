...
class PointTransformer_FP(nn.Module):
    def __init__(self, embd = 64, with_oa = True):
        ...
        self.fp2 = PointNetFeaturePropagation(in_channel=(embd*4*4*2 + embd*2), mlp=[embd*4*2, embd*4*2])
        self.fp1 = PointNetFeaturePropagation(in_channel=(embd*4*2 + embd), mlp=[embd*2, embd*2])

        self.linear = nn.Conv1d(embd*2, embd, kernel_size=1)
